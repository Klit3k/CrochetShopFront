import AdminNavbar from "../navigation/AdminNavbar";
import {useState, useEffect, useCallback, updateState} from 'react';
import axios from 'axios';


function EditProduct() {
    const [product, setProduct] = useState({
        productId: 0,
        name: "",
        description: "",
        price: 0,
        image: {
          id: 0,
          name: "Default picture",
          file: ""
        }
      });

    const [list, setList] = useState({
        products: [],
    });
    const [alertInfo, setAlertInfo] = useState(false);
    const [state, setState] = useState({
      loaded: false,
    });

    var editProductServer = async () => {
        var response = await axios.put("http://localhost:8080/product", null, {
            params: {
                productId: Number(product.productId),
                name: product.name,
                description: product.description,
                price: parseFloat(product.price),
            }
        })
        .then(response => {
          console.log(response)
          setAlertInfo(true);
        })
        .catch( err => {
          switch (err.response.status) {
            case 403:
                setProduct({
                    name: "",
                    description: "",
                    price: 0,
                    productId: 0,
                    image: {
                      id: 0,
                      name: "Default picture",
                      file: ""
                    }
                  });
              break;
            default:
              break;
         }
        });
        return response;
      };

    var listProductServer = async () => {
    var response = await axios
      .get("http://localhost:8080/products", null, null)
      .then((response) => {
        response.data.forEach(el => {
          if(el.image == null) {
            el.image = {
              id: 0,
              name: "Default picture",
              file: ""
            }
          }
        });
        
        setList({ products: response.data });

        setState({ loaded: true });

      })
      .catch((err) => {
        switch (err.response.status) {
          case 403:
            //   setState({
            //     email: state.email,
            //     password: state.password,
            //     validData: "Invalid credentials"
            //   });
            break;
          default:
            break;
        }
      });
    };

    useEffect(() => {
        listProductServer();
      }, [state.loaded]);

    const handleInputChange = (event) => {
      const value = event.target.value;
        setProduct({
          ...product,
          [event.target.name]: value
       })
    };

    const handleSubmit = (event) => {
        event.preventDefault();
        editProductServer(); 
        setState({loaded: false});
        listProductServer();

        
    };
    
    const handleControlChange = (event) => {

      if(list.products.length > 0 && list.products.find(el => {return el.id == event.target.value ? el : null}) !== undefined){
        setProduct({
          productId: event.target.value,
          name: list.products.find(el => {return el.id == event.target.value? el.name : null}).name,
          description: list.products.find(el => {return el.id == event.target.value ? el.description : null}).description,
          price: list.products.find(el => {return el.id == event.target.value ? el.price : null}).price,
          image: list.products.find(el => {return el.id == event.target.value ? el.image : null}).image
        })
      }
      listProductServer();
    };
  return !state.loaded ? null :  (
    <div className="AddProduct">
      
        <div className="container">
        <div className="mx-auto d-flex justify-content-center">
              <h3>Edycja produktu</h3>
            </div>
          <div className="mx-auto">
          
          </div>
        </div>
        <div className="container">
        <form onSubmit={handleSubmit}>
           
           <div className="form-row">
            <div className="mb-3 form-check col-3 mx-auto">
            { alertInfo &&
                <div class="alert alert-success  text-center" role="alert">
                Produkt został edytowany.
                </div>
                }  
              <label htmlFor="productId">Wybierz produkt</label>
              <select className="form-select" onChange={handleControlChange} value={product.productId } name="productId">
                { list.products.map((e) => {       
                return (
                <option key={e.id} value={e.id}>{e.id}:{e.name} </option>
                )})}
              </select>
            </div>
            
            <div className="mb-3 col-3 mx-auto">
              {list.products.length > 0 && product.image !== undefined ? 
              <img src={"data:image/png;base64,"+product.image.file} alt={product.image.name} className="img-thumbnail"></img>
              : null}
            </div>

            </div>
            <div className="mb-3 form-check col-3 mx-auto">
                <label htmlFor="name" className="form-check-label">Nazwa</label>
                <input type="text" name="name" onChange={handleInputChange} value={product.name} className="form-control"/> 
            </div>

            <div className="mb-3 form-check col-3 mx-auto">
                <label htmlFor="description" className="form-check-label">Opis </label>
                <textarea type="text" name="description" onChange={handleInputChange} value={product.description} className="form-control"/>
            </div>

            <div className="mb-3 form-check col-3 mx-auto">
                <label htmlFor="price" className="form-check-label">Cena</label>
                <input type="number" step={0.01} name="price" onChange={handleInputChange} value={product.price}  className="form-control"/>
            </div>

             <div className="d-flex justify-content-center">
                <button type="submit" className="btn btn-primary mb-5" >Edytuj produkt</button>
            </div>
        </form>
        </div>
    </div>
  );
}
export default EditProduct;
