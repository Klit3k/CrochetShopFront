import React, { useEffect, useState } from 'react'
import { NavLink, useOutletContext, useParams } from 'react-router-dom'
import GetUser from '../shopping/fetchers/getUser'
const OrderDetails = () => {
    const { orderId } = useParams()
    const client = GetUser()
    const [state, setState ] = useState(false)
    const [order, setOrder] = useState(null)

    useEffect(() => {
      if(client){
        setOrder(
            client.orders.find((e) => {
                return e.payuOrderId === orderId ? e : null})
        )
        console.log(order)
        setState(true)
      }
    
      

    }, [client])
    

    return !state ? null : (
        <>
        <div className='container mt-2 d-flex justify-content-start col-5'>
        <NavLink to="/home/account"  > <button className="btn btn-outline-dark">Wróć do zamówień</button></NavLink>
        </div>
        <div className="container mt-3">
            <div className='row'>
            <div className='mx-auto d-flex justify-content-center'>
            <div class="col-md-4 order-md-2 mb-4">
            <div class="d-flex justify-content-center text-center mb-3 row">
            <h5>Szczegóły zamówienia <br/>#{order.payuOrderId}</h5>
            <span>Status zamówienia: {`${(order.status).charAt(0).toUpperCase()}${order.status.slice(1).toLowerCase()}`}</span>
            </div>                       
             <ul class="list-group mb-3">

            {order.products.map((product) => {
                return (
                            <li class="list-group-item d-flex justify-content-between lh-condensed">
                                <div className='col-5'>   
                                {
                                    product.image !== undefined ?
                                    
                                    <img className="img-fluid rounded" 
                                    src={
                                        'data:image/png;base64,' +
                                        product.image.file
                                    }
                                    alt={product.image.name}
                                    ></img>
                                    :
                                    <img className='img-fluid rounder'
                                        alt="Default picture"
                                        src={'data:image/png;base64,'+""}
                                        >
                                    </img>
                                   
                                }
                                    
                                </div>
                                <div className='col-5'>
                                    <h6 class="my-0">{product.name}</h6>
                                    <small class="text-muted">
                                        {product.description.slice(0,20)}
                                    </small>
                                </div>
                                <span class="text-muted">{product.price} zł</span>
                            </li>
                            
                )
            })}
            <li class="list-group-item d-flex justify-content-between">
                                <span> <b>Suma</b> </span>
                                <strong>{order.value} zł</strong>
                            </li>
                        </ul>
                </div>
            </div>
            </div>
        </div>
        </>
    )
}

export default OrderDetails
